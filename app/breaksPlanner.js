...
class BreaksPlanner extends EventEmitter {
  constructor(settings) {
    ...
    this.naturalBreaksManager.on("clearBreakScheduler", () => {
      if (
        !this.isPaused &&
        this.scheduler.reference !== "finishMicrobreak" &&
        this.scheduler.reference !== "finishBreak" &&
        this.scheduler.reference !== null
      ) {
        this.clear();
    ...
    if (!shouldBreak && shouldMicrobreak) {
      if (microbreakNotification) {
        this.scheduler = new Scheduler(
          () => this.emit("startMicrobreakNotification"),
          interval - microbreakNotificationInterval,
      ...
      if (breakNotification) {
        this.scheduler = new Scheduler(
          () => this.emit("startBreakNotification"),
          interval * (this.settings.get("breakInterval") + 1) - breakNotificationInterval,
      ...
      if (this.breakNumber % breakInterval === 0) {
        if (breakNotification) {
          this.scheduler = new Scheduler(
            () => this.emit("startBreakNotification"),
            interval - breakNotificationInterval,
            "startBreakNotification",
          );
        } else {
  ...
  postponeCurrentBreak() {
    ...
    if (
      notification &&
      this.settings.get(`${scheduledBreakType}PostponeTime`) > this.settings.get(`${scheduledBreakType}NotificationInterval`)
    ) {
      postponeTime =
        this.settings.get(`${scheduledBreakType}PostponeTime`) - this.settings.get(`${scheduledBreakType}NotificationInterval`);
