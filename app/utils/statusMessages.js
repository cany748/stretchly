...
class StatusMessages {
  ...
  get trayMessage() {
    ...
    if (this.isPaused) {
      if (this.timeLeft) {
        message +=
          i18next.t("statusMessages.paused") +
          " - " +
        ...
        message += i18next.t("statusMessages.paused") + " " + i18next.t("statusMessages.indefinitely");
        return message;
      }
    }
    ...
    const breakNotificationInterval = this.settings.get("breakNotification") ? this.settings.get("breakNotificationInterval") : 0;
    ...
    if (this.reference === "startBreak") {
      message += i18next.t("statusMessages.nextLongBreak") + " " + Utils.formatTimeIn(this.timeLeft, this.settings.get("language"));
      return message;
    }

    if (this.reference === "startMicrobreak") {
      message += i18next.t("statusMessages.nextMiniBreak") + " " + Utils.formatTimeIn(this.timeLeft, this.settings.get("language"));
    ...
    if (this.reference === "startMicrobreakNotification") {
      message +=
        i18next.t("statusMessages.nextMiniBreak") +
        " " +
