const { app, nativeTheme, BrowserWindow, Menu, ipcMain, shell, dialog, globalShortcut, Tray } = require("electron");
...
function trayIconPath() {
  const params = {
    paused:
      breakPlanner.isPaused || breakPlanner.naturalBreaksManager.isSchedulerCleared || breakPlanner.appExclusionsManager.isSchedulerCleared,
...
function startMicrobreak() {
  ...
  const postponable = settings.get("microbreakPostpone") && breakPlanner.postponesNumber < postponesLimit && postponesLimit > 0;
...
function startBreak() {
  ...
  const postponable = settings.get("breakPostpone") && breakPlanner.postponesNumber < postponesLimit && postponesLimit > 0;
  const showBreaksAsRegularWindows = settings.get("showBreaksAsRegularWindows");
...
function loadIdeas() {
  ...
  if (settings.get("useIdeasFromSettings")) {
    ...
    miniBreakIdeasData = Object.keys(t("miniBreakIdeas", { returnObjects: true })).map((item) => {
      return { data: i18next.t(`miniBreakIdeas.${item}.text`), enabled: true };
    });

    longBreakIdeasData = Object.keys(t("longBreakIdeas", { returnObjects: true })).map((item) => {
...
function createPreferencesWindow() {
  ...
  const maxHeight = electron.screen.getDisplayNearestPoint(electron.screen.getCursorScreenPoint()).workAreaSize.height * 0.9;
...
ipcMain.on("show-debug", function (event) {
  ...
  event.sender.send("debugInfo", reference, timeleft, breaknumber, postponesnumber, settingsFile, logsFile);
});
