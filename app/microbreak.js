...
window.onload = (e) => {
  ...
  document.querySelector("#close").onclick = (event) => ipcRenderer.send("finish-microbreak", false);

  document.querySelector("#postpone").onclick = (event) => ipcRenderer.send("postpone-microbreak");
  ...
  ipcRenderer.once("progress", (event, started, duration, strictMode, postpone, postponePercent, backgroundColor) => {
    ...
    window.setInterval(() => {
      ...
      if (Date.now() - started < duration) {
        ...
        postponeElement.style.display = Utils.canPostpone(postpone, passedPercent, postponePercent) ? "flex" : "none";
        closeElement.style.display = Utils.canSkip(strictMode, postpone, passedPercent, postponePercent) ? "flex" : "none";
        progress.value = ((100 - passedPercent) * progress.max) / 100;
        progressTime.innerHTML = Utils.formatTimeRemaining(Math.trunc(duration - Date.now() + started), settings.get("language"));
      }
    }, 100);
  });
};
