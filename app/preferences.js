...
window.onload = (e) => {
  ...
  ipcRenderer.on("debugInfo", (event, reference, timeleft, breaknumber, postponesnumber, settingsfile, logsfile) => {
  ...
  ipcRenderer.on("renderSettings", (event, settings) => {
    document.querySelectorAll('input[type="checkbox"]').forEach((checkbox) => {
      ...
      if (!eventsAttached) {
        checkbox.onchange = (event) => ipcRenderer.send("save-setting", checkbox.value, isNegative ? !checkbox.checked : checkbox.checked);
      }
    });
    ...
    document.querySelectorAll('input[type="range"]').forEach((range) => {
      ...
      document.querySelector("#longBreakEvery").closest("div").querySelector("output").innerHTML = i18next.t("utils.minutes", {
        count: parseInt(realBreakInterval()),
      });
      if (!eventsAttached) {
    ...
    document.querySelectorAll(".sounds img").forEach((preview) => {
      if (!eventsAttached) {
        preview.onclick = (event) => ipcRenderer.send("play-sound", preview.closest("div").querySelector("input").value);
      }
    });
